...
minReqFileName = strcat('..\Data\Minimum Requirements\', sheetName, '_0.xls');
...
for i=1:1:length(model.rxns)
    ...
    if ~isempty(result{i,5})
        if model.lb(i) == 0  % Irreversible only
            % If transcriptome read not less than minimal requirements for
            % growth
            if model.ub(i) < str2double(FbaMinData(i+1))
...
% GET EXPERIMENTAL MEDIUM DATA FOR TRANSPORT REACTIONS
...
% Save results
excelFileName = convertStringsToChars(strcat('../Results post-/Threshold', string(zeroGeneExpressionThreshold),'/', sheetName, '_0.xls'));
